...
name:     sass-awesome-docs-first-sass
title:    Sass 100% 이해하기 1번째
subtitle: Hello!
tags:     sass, scss, css
...
## 구성
- 어떻게 보면 좋은지, 책 참조
- 문서에 기초, 쉬운 용어로 풀어쓰고 예시 추가
- 어떨까 의문 해결 - 문서에는 없는 경우의 수에 따라 어떻게 되는지 추가 설명
- 관련내용 모음 - 컨텐츠 연결, 연관부분 모아서 보여주기
- 한눈에 보기 - 표나 목록으로 재정리해서
...
정의(Definition) http://egloos.zum.com/dstein/v/2563434 선언과 정의 차이
컴파일러에게 대상의 실제 내용을 생성하게한다.
실제 내용을 생성하고 할당하므로 메모리를 사용한다.
...
#### 키워드 인자 http://blog.daum.net/swrush/155 -> 믹스인과 유사함
...
이 지시자는 믹스인 이름을 받으며, 선택적으로 인자를 보낼 수 있다.
...
믹스인은 인자로 SassScript 값을 취할 수 있다.
이 값은 믹스인이 포함될때 주어지며, 믹스인 안에서 변수로서 사용 가능하다.

믹스인은 정의할 때, 믹스인 이름 뒤에 괄호안에 콤마로 구분한 변수 이름이 인자이다.
믹스인을 포함할때도 마찬가지로 이름 뒤 괄호안에 값을 작성한다.

믹스인은 정의할 때, 인자는 기본값을 지정하려면 일반 변수 설정 문법을 사용한다.
믹스인을 포함할때 인자 값을 써넣지 않으면 기본값이 대신 사용된다.

#### 키워드 인자
http://sass-lang.com/documentation/file.SASS_REFERENCE.html#keyword_arguments_2
믹스인은 포함할때 명시적 키워드 인자 explicit keyword arguments를 사용할 수 있다 (변수 이름 같이 쓰는 것)

덜 간결하지만 스타일시트를 읽기 더 쉽게 한다
그리고 많은 인자를 제공하는 함수가 더 유연하게 인터페이스를 표현하여 여러움없이 호출할 수 있다
  
명명된 인자 Named arguments 는 순서와 상관없이 작성할 수 있다
기본값을 가진 인자는 생략할 수 있다
명명된 인자가 변수 이름이므로, 하이픈과 언더스코어를 교차하여 사용할 수 있다. *믹스인, 변수에서도 나옴3

#### 변수 인자
http://sass-lang.com/documentation/file.SASS_REFERENCE.html#variable_arguments
어떤 경우에는 믹스인과 함수가 알수 없는 수의 인자를 받아야할 때도 있다
예로 `box-shadow`를 만드는 믹스인은 인자로 여러 쉐도우 값을 받을 수 있다
이런 경우를 위해 Sass는 "변수 인자 variable arguments"를 지원한다.
믹스인이나 함수 선언 끝에 위치한 인자로 남은 모든 인자를 목록 데이터타입으로 묶어서 받는다.
 이 인자는 일반 인자와 똑같이 보이지만 뒤에 `...`이 따른다
 * 값이 하나라도 목록형인가? / ()형 목록은 어떻게 ? / @each 를 쓸수 있나?
 
변수 인자는 키워드 인자도 포함하여 믹스인이나 함수에 넘길 수 있다
이는 `keywords($args)` 함수를 사용해서 구현하는데, 이 함수는 $없는 문자열을 값으로 바꿔 맵을 반환한다.

변수 인자는 믹스인은 호출할 때도 사용할 수 있다.
같은 문법을 사용해, 값의 목록을 확장하여 각 값이 분리된 인자로 넘겨진다.
또는 값의 맵으로 확장할 수도 있으며 각 이름-값 쌍은 각 키워드 인자로 취급된다
* 하나의 변수에 여러 값을 모아서 넘긴다 라고 알려줌

목록 하나와 맵 하나의 인자를 같이 넘길 수도 있는데, 목록이 먼저오고 맵이 나중에 와야한다
`@include colors($values..., $map...)`
* 왜? 키워드 인자와 일반 인자도 그런가, 일반 값은 순서대로 이후 키워드는 마지막에 와야하나부다

변수 인자는 특정 믹스인의 인자 시그니처 argument signature를 바꾸지 않으면서 스타일을 추가하기 위한 중첩 믹스인에서 사용할 수 있다.
이를 위해선 키워드 인자는 바로 감싸진 믹스인으로 넘겨져야 한다.
...
함수는 모든 글로벌 변수에 접근 가능(믹스인도 같음)하며 믹스인처럼 인자를 받을 수도 있다.
함수안에는 여러 구문을 가질 수 있으며, 반드시 `@return`을 이용해 함수의 값을 반환해야 한다.
 
 믹스인과 같이, Sass 정의 함수도 키워드 인자를 사용할 수 있다.
사용자 정의 함수는 믹스인과 동일하게 변수 인자를 지원한다.
